...
class WorkflowContext
  ...
  def add_action(name)
    ...
    if action_class_is_present?(name)
      require get_action_class_filename(name)
    else
      raise "Workflow action class for #{name} is not present."
    end
    
    # get action class object
    actionClassObject = Object.const_get(get_action_class_name(name))
    ...
    self.class.send(:define_method, name.to_sym) { |*args|
      ...
      # save current state
      wr = WorkflowRecord.find(:first, :conditions => {:source_id => @source_id})
      wr.state = name
      wr.save
    }
  end
  ...
  def action_class_is_present?(name)
    File.exist?(get_action_class_filename(name))
  end

  # tanslate action name into action class filename
  # * name: class name
  def get_action_class_filename(name)
    File.join(File.dirname(__FILE__), (name + "_action.rb").downcase)
  end
  
  # tanslate action name into action class name
  # * name: class name
  def get_action_class_name(name)
