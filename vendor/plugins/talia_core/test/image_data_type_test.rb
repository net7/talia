...
module TaliaCore
  ...
  class ImageDataTypeTest < Test::Unit::TestCase
    ...
    def test_mime_types
      ...
      assert_equal("image/jpeg", @test_records[8].mime_type)
      assert_equal("image/png", @test_records[9].mime_type)
      assert_equal("image/tiff", @test_records[10].mime_type)
    end
    ...
    def test_data_directory
      ...
      assert_equal(File.join(base_dir_name, 'temp1.bmp'), File.join(dir_for_test, @test_records[5].location))
      assert( File.exists?(File.join(dir_for_test, @test_records[5].location)), "#{File.join(dir_for_test, @test_records[5].location)} does not exist" )
    end

    # test file size
    def test_file_size
      assert_equal(1254, @test_records[5].size)
    end
    
    # test binary access
    def test_binary_access
      # Check initial position
      assert_equal(0, @test_records[5].position)
      
      # Try to read all bytes
      bytes = @test_records[5].all_bytes
      assert_equal(1254, bytes.size)
      assert_equal(false, @test_records[5].is_file_open?)
