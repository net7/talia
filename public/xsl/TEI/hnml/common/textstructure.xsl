...
<xsl:stylesheet
 ...
 version="1.0">
  <xd:doc type="stylesheet">
    ...
    <xd:cvsId>$Id: textstructure.xsl,v 1.1 2006/02/16 15:31:44 giacomi Exp $</xd:cvsId>
    ...
    <xsl:choose>
      ...
      <xsl:when test="ancestor::front">
        <xsl:if test="not($numberFrontHeadings='')">
          <xsl:number format="{$numberFrontHeadings}" level="multiple" from="front" count="div|div0|div1|div2|div3|div4"/>
          <xsl:value-of select="$numbersuffix"/>
        </xsl:if>
      </xsl:when>
      <xsl:when test="ancestor::back">
        <xsl:if test="not($numberBackHeadings='')">
          ...
          <xsl:text> </xsl:text>
          <xsl:number format="{$numberBackHeadings}" level="multiple" from="back" count="div|div0|div1|div2|div3|div4"/>
          <xsl:value-of select="$numbersuffix"/>
        </xsl:if>
      </xsl:when>
      <xsl:otherwise>
        <xsl:number level="multiple" from="body" count="div|div0|div1|div2|div3|div4"/>
        <xsl:value-of select="$numbersuffix"/>
      </xsl:otherwise>
    </xsl:choose>
  </xsl:template>
