...
<xsl:stylesheet 
  ...
  <xsl:template name="generateAuthor">
	...
	<xsl:for-each
	    select="ancestor-or-self::TEI.2/teiHeader/fileDesc/titleStmt/author">
	  <xsl:apply-templates/>
	  <xsl:choose>
  ...
  <xsl:template name="generateRevAuthor">
    <xsl:variable name="who">
      <xsl:choose>
	<xsl:when
	    test="ancestor-or-self::TEI.2/teiHeader/revisionDesc/@vcwho">
	  <xsl:apply-templates 
	      select="ancestor-or-self::TEI.2/teiHeader/revisionDesc/@vcwho"/>
	</xsl:when>
	<xsl:when test="ancestor-or-self::TEI.2/teiHeader/revisionDesc/change[1]/respStmt/name">
	  <xsl:value-of
  ...
  <xsl:template name="generateRevDate">
    ...
    <xsl:choose>
      <xsl:when
	  test="ancestor-or-self::TEI.2/teiHeader/revisionDesc/@vcdate">
	<xsl:apply-templates 
	    select="ancestor-or-self::TEI.2/teiHeader/revisionDesc/@vcdate"/>
      </xsl:when>
  ...
  <xsl:template name="generateDate">
	...
	<xsl:when test="$useHeaderFrontMatter='true' and ancestor-or-self::TEI.2/text/front//docDate">
	  <xsl:apply-templates select="ancestor-or-self::TEI.2/text/front//docDate" mode="date"/>
	</xsl:when>
	<xsl:when test="ancestor-or-self::TEI.2/teiHeader/fileDesc/publicationStmt/date">
	  <xsl:apply-templates select="ancestor-or-self::TEI.2/teiHeader/fileDesc/publicationStmt/date"/>
	</xsl:when>
  ...
  <xsl:template name="generateTitle">
	...
	<xsl:apply-templates select="ancestor-or-self::TEI.2/text/front//docTitle"/>
      ...
      <xsl:when test="$useHeaderFrontMatter='true' and ancestor-or-self::teiCorpus.2/text/front//docTitle">
	...
	<xsl:for-each select="ancestor-or-self::TEI.2|ancestor-or-self::teiCorpus.2">
	  <xsl:apply-templates select="teiHeader/fileDesc/titleStmt/title" mode="htmlheader"/>
	</xsl:for-each>
      </xsl:otherwise>
    </xsl:choose>
  </xsl:template>
