...
$Date: 2005/03/04 14:07:22 $, $Revision: 1.20 $, $Author: rahtz $
...
<xsl:template name="generateTitle">
 <xsl:choose>
   ...
   <xsl:when test="$useHeaderFrontMatter='true' and ancestor-or-self::teiCorpus.2/text/front//docTitle">
     <xsl:apply-templates 
       select="ancestor-or-self::teiCorpus.2/text/front//docTitle"/>
     </xsl:when>
   <xsl:otherwise>
     <xsl:for-each
	 select="ancestor-or-self::TEI.2|ancestor-or-self::teiCorpus.2">
...
<xsl:template match="figure" mode="xref">
  <xsl:choose>
	...
	<xsl:when test="ancestor::front">
	  <xsl:number level="any"   count="figure[head]" from="front"/>
	</xsl:when>
	<xsl:when test="ancestor::back">
	  <xsl:number level="any"  count="figure[head]" from="back"/>
	</xsl:when>
	<xsl:when test="ancestor::body" >
	  <xsl:number level="any"   count="figure[head]" from="body"/>
	</xsl:when>
      </xsl:choose>
      <xsl:if test="head">
	<xsl:text>, </xsl:text>
