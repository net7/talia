...
<xsl:stylesheet 
  ...
  <xsl:key name="GIs" match="elementSpec" use="@ident"/>
  ...
  <xsl:key name="Modules"   match="moduleSpec" use="1"/>
...
&lt;!ENTITY % file.<xsl:value-of select="@ident"/>-decl PUBLIC '-//TEI P5//<xsl:value-of select="altIdent[@type='FPI']"/>//EN' '<xsl:value-of select="@ident"/>-decl.dtd' &gt;
...
<xsl:if test="@type='core'">
	  <xsl:for-each select="key('AllModules',1)">
	    ...
	    <xsl:if test="not(@type='core')">
&lt;![%TEI.<xsl:value-of select="@ident"/>;[
&lt;!ENTITY % file.<xsl:value-of select="@ident"/> PUBLIC '-//TEI P5//<xsl:value-of select="altIdent[@type='FPI']"/>//EN' '<xsl:value-of select="@ident"/>.dtd' &gt;
...
<xsl:template match="macroSpec[@id='TEIGIS' or @xml:id='TEIGIS']" mode="tangle"/>
...
<xsl:template match="elemDecl" mode="literal">
...
<xsl:template match="rng:element">
  <xsl:choose>
    <xsl:when test="parent::content/parent::macroSpec">
...
<xsl:template match="macroSpec[@type='dt']/content/rng:text">
  <xsl:text> CDATA</xsl:text>              
</xsl:template>

<xsl:template match="macroSpec[@type='epe']/content/rng:text">
  <xsl:text>CDATA</xsl:text>              
</xsl:template>
...
<xsl:template match="rng:text" mode="simple">
  <xsl:choose>
    <xsl:when test="parent::content/parent::macroSpec[@type='dt']">
      <xsl:text> CDATA</xsl:text>              
    </xsl:when>
    <xsl:when test="parent::content and not(following-sibling::rng:*) and not (preceding-sibling::rng:*)">
      <xsl:text>(#PCDATA)</xsl:text>
    </xsl:when>
    <xsl:when test="parent::content and not(following-sibling::rng:*) and not (preceding-sibling::rng:*)">
      <xsl:text>(#PCDATA)</xsl:text>
...
<xsl:template match="gloss|remarks|desc"/>
...
&lt;!ENTITY </xsl:text>   
  ...
  <xsl:for-each select="content">
    <xsl:apply-templates
     select="*|rng:*|processing-instruction()"/>
  </xsl:for-each>
  <xsl:text>'</xsl:text>
  <xsl:text> &gt;</xsl:text>
...
&lt;!ATTLIST </xsl:text>
  ...
  <xsl:apply-templates select="classes/memberOf" mode="tangleAtts"/>
...
&lt;!ENTITY </xsl:text>   
  ...
  <xsl:value-of select="key('FILES',@key)/altIdent[@type='FPI']"/>
...
<xsl:template match="classSpec" mode="tangle">
  <xsl:message>    .. classSpec <xsl:value-of select="@id"/>,<xsl:value-of select="@type"/>  </xsl:message>
  <xsl:choose>
    <xsl:when test="@type='atts'">
...
<xsl:template match="classSpec" mode="process">
  <xsl:choose>    
    <xsl:when test="@type='atts'">
      <xsl:call-template name="classAtt"/>
    </xsl:when>
...
<xsl:template match="commDecl" mode="tangle">
  <xsl:text>
  &lt;!--</xsl:text>
  <xsl:apply-templates/>
  <xsl:text>--&gt;</xsl:text>        
...
<xsl:text>   </xsl:text>
	...
	<xsl:for-each select="valList/valItem">
	  <xsl:value-of select="@ident"/>
	  <xsl:if test="following-sibling::valItem">|</xsl:if> 
	</xsl:for-each>
	<xsl:text>)</xsl:text>
      </xsl:when>
      <xsl:otherwise>
    ...
    <xsl:choose>
      <xsl:when test="string-length(defaultVal) &gt;0">
	<xsl:text> &#34;</xsl:text>
