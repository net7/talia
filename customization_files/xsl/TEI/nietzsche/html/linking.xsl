...
<xsl:stylesheet 
  ...
  <xd:doc type="stylesheet">
    ...
    <xd:cvsId>$Id: linking.xsl,v 1.1 2006/02/16 15:31:45 giacomi Exp $</xd:cvsId>
...
<xd:doc>
  ...
  <xsl:template match="label|figure|table|item|p|bibl|anchor|cell|lg|list|sp" mode="generateLink">
    <xsl:variable name="ident">
      <xsl:apply-templates select="." mode="ident"/>
    </xsl:variable>
    <xsl:variable name="file">
      <xsl:apply-templates select="ancestor::*[starts-with(local-name(),'div')][1]" mode="generateLink"/>
    </xsl:variable>
    <xsl:choose>
