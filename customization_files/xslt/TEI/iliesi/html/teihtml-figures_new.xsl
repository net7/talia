...
<!-- Added by ILIESI-->
      <xsl:when test="@n">
        <xsl:text>http://151.100.146.63/img/icon_img.jpg</xsl:text>
      </xsl:when>
<!-- end     -->
      <xsl:when test="@url">
  ...
  <xsl:choose>
    <xsl:when test="$showFigures='true'">
      <xsl:choose>
       <xsl:when test="@n">
         <a href="#_{$Img}" onClick="javascript:window.open('{$Link}', '', 'top=10,left=10,width=500,height=400,status=no,menubar=no,toolbar=no,scrollbars=1');" >        
          <img src="{$graphicsPrefix}{$File}" border="0"></img></a>
      </xsl:when>
      <xsl:otherwise>
